 ...
 */
export function calculateHash(text: string): number {
  let hash = 0;
 ...
 */
export function generateRgbFromHash(hash: number): RGB {
 ...
 */
export function calculateTextContrast(rgb: RGB): number {
 ...
 */
export function getStableColor(text: string): {
 ...
 */
export function getStableTextColor(text: string): string {
  const hash = calculateHash(text);
  const rgb = generateRgbFromHash(hash);
  return rgbToHex(rgb);
}
