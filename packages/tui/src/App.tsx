...
import { ExploreTrackerService, TranscriptReader, ParsedTranscriptEntry, PlanFile, Task } from '@agent-tracker/core';
...
export function App() {
  ...
  switch (currentView.type) {
    ...
    case 'tasks-list': {
      const tasksListView = currentView as Extract<NavStackItem, { type: 'tasks-list' }>;
      return (
        <TasksListView
    ...
    case 'task-set-detail': {
      const taskSetDetailView = currentView as Extract<NavStackItem, { type: 'task-set-detail' }>;
      return (
        <TaskSetDetailView
          conversationId={taskSetDetailView.conversationId}
          selectedTaskId={taskSetDetailView.selectedTaskId}
          onSelectTask={navigation.selectTask}
          onViewTask={(task: Task) =>
            navigation.pushTaskDetail(taskSetDetailView.conversationId, task)
          }
          onBack={navigation.pop}
        />
      );
    ...
    case 'task-detail': {
      const taskDetailView = currentView as Extract<NavStackItem, { type: 'task-detail' }>;
      return (
        <TaskDetailView
          task={taskDetailView.task}
          conversationId={taskDetailView.conversationId}
          onBack={navigation.pop}
        />
      );
    }
