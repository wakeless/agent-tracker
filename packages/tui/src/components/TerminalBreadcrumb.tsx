...
import { Session, parseITermSessionId } from '@agent-tracker/core';
 ...
 */
export function TerminalBreadcrumb({ session }: TerminalBreadcrumbProps) {
  const isITerm = session.terminal.term_program === 'iTerm.app';

  if (isITerm) {
    return <ITermBreadcrumb session={session} />;
  }

  return <GenericTerminalBreadcrumb session={session} />;
}

function ITermBreadcrumb({ session }: TerminalBreadcrumbProps) {
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      ...
      {hasProfile && (
        <Box marginLeft={10}>
          <Text dimColor>iTerm2 | {session.terminal.iterm.profile}</Text>
        </Box>
      )}
    </Box>
  );
...
function GenericTerminalBreadcrumb({ session }: TerminalBreadcrumbProps) {
