...
export function SessionDetail({ session, recentTranscript = [] }) {
    ...
    return (React.createElement(Box, { flexDirection: "column", padding: 1 },
        ...
        session.terminal && (session.terminal.pid || session.terminal.ppid) && (React.createElement(React.Fragment, null,
            React.createElement(Box, { marginTop: 1, marginBottom: 1 },
                React.createElement(Text, { bold: true, underline: true }, "Process Information")),
            session.terminal.pid && session.terminal.pid !== 'unknown' && (React.createElement(DetailRow, { label: "Process ID (PID)", value: session.terminal.pid })),
            session.terminal.ppid && session.terminal.ppid !== 'unknown' && (React.createElement(DetailRow, { label: "Parent PID (PPID)", value: session.terminal.ppid })))),
        session.workSummary && (React.createElement(Box, { marginTop: 1 },
            React.createElement(Text, { bold: true }, "Current Work: "),
            React.createElement(Text, { italic: true, color: "magenta" }, session.workSummary))),
